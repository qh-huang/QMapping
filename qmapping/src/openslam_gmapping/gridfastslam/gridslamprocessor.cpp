...
# endif
  ...
  GridSlamProcessor::~GridSlamProcessor(){
    ...
    cerr << __FUNCTION__ << ": Deleting tree" << endl;
  ...
  void GridSlamProcessor::processTruePos(const OdometryReading& o){
	...
	bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
		...
		if (! m_count 
			...
			// debug
			std::cout << "plainReadings = ";
			for (size_t i = 0; i < m_beams; i++) {
				std::cout << plainReading[i] << " ";
			}
			std::cout << std::endl;
			// debug end
			...
			if (m_count>0){
//				if (move.theta > 0.01){
//					std::cout << "ProcessScan: skip scanmatch while rotation!" << std::endl;
//					return processed;
//				}
				scanMatch(plainReading);
				if (m_infoStream){
					m_infoStream << "LASER_READING "<< reading.size() << " ";
					m_infoStream << setiosflags(ios::fixed) << setprecision(2);
					for (RangeReading::const_iterator b=reading.begin(); b!=reading.end(); b++){
						m_infoStream << *b << " ";
					}
					OrientedPoint p=reading.getPose();
					m_infoStream << setiosflags(ios::fixed) << setprecision(6);
					m_infoStream << p.x << " " << p.y << " " << p.theta << " " << reading.getTime()<< endl;
					m_infoStream << "SM_UPDATE "<< m_particles.size() << " ";
					for (ParticleVector::const_iterator it=m_particles.begin(); it!=m_particles.end(); it++){
  ...
  void GridSlamProcessor::scanMatch(const double* plainReading){
	...
	for (ParticleVector::iterator it = m_particles.begin(); it != m_particles.end(); it++){
		...
		if (score>m_minimumScore){
			it->pose = corrected;
			if (m_infoStream){
				m_infoStream << "Scan Matching Success! Likelihood=" << score << std::endl;
			}
		} else {
			if (m_infoStream){
				m_infoStream << "Scan Matching Failed, using odometry. Likelihood=" << score << std::endl;
